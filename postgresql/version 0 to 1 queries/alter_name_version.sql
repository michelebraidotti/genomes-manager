...
SELECT 'UPDATE sequence.sequences SET name = ''' || sc.name || ''', version = ''' || sc.scaff_version || ''' WHERE id = ' || s.id || ';'
  FROM sequences s join scaffolds sc on s.id = sc.sequence_id;
SELECT 'UPDATE sequence.sequences SET name = ''' || p.name || ''', version = ''' || p.version || ''' WHERE id = ' || s.id || ';'
  FROM sequences s join pseudomolecules p on s.id = p.sequence_id;
